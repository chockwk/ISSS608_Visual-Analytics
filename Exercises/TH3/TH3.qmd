...
-   `lubridate` handles date and time.
...
```{r}
pacman::p_load(tidyverse, ggplot2, plotly, lubridate)
```

# 3. Import Data

Among the weather stations, Changi station has been traditionally treated as a rural area station and has the longest series of temperatures and rainfall records since 1982. Newton station is located in a dense area with high rise residential and commercial buildings. Jurong is a residential area in the west of Singapore. Pulau Ubin and Sentosa are small islands close to the main island of Singapore.
...
derive Mean Monthly Rainfall, Max Monthly Temperature and Minimum Monthly Temperature.
...
ggplot(jurong, aes(x = as.Date(paste(Year, Month, Day, sep = "-")),
  ...
  geom_label_repel(aes(label = ifelse(Daily.Rainfall.Total..mm. > 150, 
                                      as.character(Date), "")),
                   na.rm = TRUE,
                   box.padding = 0.35, 
                   point.padding = 0.5,
...
A heatmap could be useful for visualizing the intensity of rainfall across the days of the month. Each cell of the heatmap represent a period of 10 years, with the color intensity indicating the amount of rainfall.
...
ggplot(j10_melted, aes(x = Year, y = Month, fill = value)) + 
  geom_tile() + 
  labs(title = "Heatmap of Daily Rainfall: 1983-2023", x = "Year", y = "Month", fill = "Rainfall (mm)") +
  theme_minimal()
...
# Assuming 'j10' is your dataset and it contains the variables 'Daily.Rainfall.Total..mm.',
# 'Maximum.Temperature', 'Year', 'Month', 'Day', and 'Date' in the appropriate formats
