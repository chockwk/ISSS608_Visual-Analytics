...
`geom_treemap()` from ggplot2 plackage to draw a tree map.
`geom_treemap()` requires an `area` aesthetic and does not interpret x and y; axes are not meaningful in a treemap. 
```{r}
