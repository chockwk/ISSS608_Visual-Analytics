...
```{r}
pacman::p_load(tidyverse, ggplot2, plotly, lubridate, ggrepel, reshape2, scales)
```
...
```{r}
p <- ggplot(j10_rain_melted, aes(x = Day, y = Year, fill = value)) + 
  geom_tile(color = "white") + 
  theme_tufte(base_family = "Helvetica") + 
  scale_fill_gradient(name = "°C ",
                      low = "lightblue",
                      high = "steelblue") +
  labs(title = "Calendar Heatmap of Daily Rainfall: 1983-2023", x = "Year", y = "Day", fill = "Rainfall (mm)") +
  theme_minimal() +
...
```{r}
p <- ggplot(j10_temp_filtered, aes(x = Day, y = Year, fill = value)) + 
  geom_tile(color = "white") + 
  theme_tufte(base_family = "Helvetica") + 
  scale_fill_gradient(name = "°C ",
                      low = "gold",
                      high = "goldenrod3") +
  labs(title = "Calendar Heatmap of Mean Temperature: 2013-2023", x = "Year", y = "Day", fill = "Temperature") +
  theme_minimal() +
  theme(axis.ticks = element_blank(), 
       plot.title = element_text(hjust = 0.5),
       legend.title = element_text(size = 8),
